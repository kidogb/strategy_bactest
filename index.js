...
class Ga {
    ...
    async fitnessApi(testsSeries) {
        ...
        const results = await this.queue(testsSeries, numberOfParallelQueries, async (data) => {
            ...
            const config = outconfig.RSI_Bull_Bear_Adx_M1_Stop_Loss
            const requestBody = {
                "params": {
                    "RSI_BULL": config['BULL_RSI'],
                    "RSI_BULL_TF": config['BULL_RSI_Timeframe'],
                    "RSI_BULL_HIGH": config['BULL_RSI_high'],
                    "RSI_BULL_LOW": config['BULL_RSI_low'],
                    "RSI_BEAR": config['BEAR_RSI'],
                    "RSI_BEAR_TF": config['BEAR_RSI_Timeframe'],
                    "RSI_BEAR_HIGH": config['BEAR_RSI_high'],
                    "RSI_BEAR_LOW": config['BEAR_RSI_low'],
                    "SMA_FAST": config['SMA_long'],
                    "SML_SLOW": config['SMA_short'],
                    "SMA_TF": config['SMA_Timeframe'],
                    "ADX": config['ADX'],
                    "ADX_TF": config['ADX_Timeframe'],
            ...
            const body = await rp.post({
                url: `http://localhost:5000/api/v1.0/backtest`,
                json: true,
                body: requestBody,
